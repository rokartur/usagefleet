...
import { PROVIDERS, type ProviderId, ProviderMark } from "@/components/OAuthSignIn";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  ...
  return (
    <Card className="gap-0">
      <CardHeader className="border-b">
        ...
        <CardDescription>
          Providers that can sign in to <span className="text-foreground">{email}</span>. A provider
          can only be connected while its verified address matches this one.
        </CardDescription>
      </CardHeader>
      {linkError && (
      ...
      <CardContent className="flex flex-col divide-y px-0">
        {PROVIDER_IDS.map((id) => {
          ...
          return (
            <div
              key={id}
              className="flex items-center justify-between gap-4 px-(--card-spacing) py-3.5"
            >
              <div className="flex items-center gap-2">
