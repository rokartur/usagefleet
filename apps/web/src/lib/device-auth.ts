...
export async function authenticateDevice(
	...
	if (!token) {
		...
		if (!rl.ok) {
			return { response: tooMany(rl.retryAfter) }
		}
		return {
			response: Response.json({ error: 'missing token' }, { status: 401 }),
		}
	}
	...
	if (missKey && budgetExhausted(missKey, BAD_TOKEN_LIMIT)) {
		return { response: tooMany(60) }
	}

	const tokenHash = hashToken(token)
	const device = (await db.select().from(devices).where(eq(devices.tokenHash, tokenHash)).limit(1))[0]
	if (!device || device.revoked) {
		if (missKey) {
			rateLimit(missKey, BAD_TOKEN_LIMIT, 60_000)
		}
		return {
