...
describe("bucket eviction", () => {
  ...
  it("keeps an active throttle alive through a unique-key flood", () => {
    ...
    expect(rateLimit(victim, 1, 60_000).ok).toBe(true);
    expect(rateLimit(victim, 1, 60_000).ok).toBe(false); // now at its limit

    for (let i = 0; i < 12_000; i++) rateLimit(`flood:${Math.random()}:${i}`, 100, 60_000);

    expect(rateLimit(victim, 1, 60_000).ok).toBe(false);
  });
});
...
describe("budgetExhausted", () => {
  it("reports exhaustion without consuming budget", () => {
    ...
    expect(budgetExhausted(key, 2)).toBe(false);
    ...
    rateLimit(key, 2, 60_000);
    expect(budgetExhausted(key, 2)).toBe(true);
  });
});
...
describe("readJsonCapped", () => {
  ...
  it("parses a body under the cap and returns it typed by the schema", async () => {
    ...
    if (parsed.ok) expectTypeOf(parsed.value.a).toEqualTypeOf<number>();
  });
  ...
  it("reports a well-formed body of the wrong shape as 400", async () => {
    await expect(
      refusal(chunked('{"a":"not a number"}'), 1024, type({ a: "number" })),
    ).resolves.toBe(400);
  });
