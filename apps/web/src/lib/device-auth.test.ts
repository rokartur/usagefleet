...
describe("authenticateDevice", () => {
  ...
  it("authenticates a known token", async () => {
    ...
    expect("device" in res && res.device).toEqual(device);
  });
  ...
  it("throttles tokenless callers on a budget of its own, not the device's", async () => {
    ...
    for (let i = 0; i < 62; i++) {
      ...
      if ("response" in res) statuses.push(res.response.status);
    }
  ...
  it("throttles a single IP that keeps presenting bad tokens", async () => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if ("response" in res && res.response.status === 429) sawThrottle = true;
    }
    ...
    expect("device" in other && other.device).toEqual(device);
  });
  ...
  it("stops hitting the database for a throttled IP", async () => {
    ...
    expect(lookup.mock.calls.length).toBe(callsAfterThrottle);
  });
});
