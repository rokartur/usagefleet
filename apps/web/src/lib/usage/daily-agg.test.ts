...
describe('daily-agg — ledgers', () => {
	it('dailyLedger is one row per day, newest first', () => {
		...
		expect(l.map(r => r.key)).toStrictEqual(['2026-06-24', '2026-06-18', '2026-05-30'])
	...
	it('monthlyLedger rolls days into months, newest first', () => {
		...
		expect(l.map(r => r.key)).toStrictEqual(['2026-06', '2026-05'])
...
describe('daily-agg — chart buckets', () => {
	it('aggToDailyBuckets is dense, zero-filling gaps, split by group/model', () => {
		const buckets = aggToDailyBuckets(ROWS, new Date('2026-06-22T12:00:00Z'), new Date('2026-06-24T08:00:00Z'))
		// 06-22, 06-23, 06-24 → 3 dense buckets even though only the 24th has data
		expect(buckets.map(b => b.label)).toStrictEqual(['Jun 22', 'Jun 23', 'Jun 24'])
	...
	it('aggToMonthlyBuckets is dense across months with compact labels', () => {
		const buckets = aggToMonthlyBuckets(ROWS, new Date('2026-05-01T00:00:00Z'), new Date('2026-06-24T00:00:00Z'))
		expect(buckets.map(b => b.label)).toStrictEqual(['May 26', 'Jun 26'])
	...
	it('keys absent group/model as ungrouped/unknown', () => {
		...
		expect([cell.g, cell.m, cell.s, cell.d]).toStrictEqual(['ungrouped', 'unknown', 'cli', 'unknown'])
	})
	...
	it('splits cells by source and device within a day', () => {
		...
		expect([cli.d, metricValue(cli.totals, 'billable')]).toStrictEqual(['dA', 10])
		expect([desk.d, metricValue(desk.totals, 'billable')]).toStrictEqual(['dB', 4])
	})
})
