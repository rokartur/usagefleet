...
import { cellKey } from './timeline'
import type { TimelineBucket, TimelineCell } from './timeline'
...
function fillBucket(ts: string, label: string, rows: DailyAggRow[]): TimelineBucket {
	...
	for (const r of rows) {
		...
		if (!cell) {
			cellAcc.set(ck, (cell = { d: dk, g: gk, m: mk, s: sk, totals: emptyTotals() }))
		}
		addInto(cell.totals, r)
	}
	return { byGroup, byModel, cells: [...cellAcc.values()], label, totals, ts }
}
...
export function groupTotals(rows: DailyAggRow[], pred?: (r: DailyAggRow) => boolean): Map<string, TokenTotals> {
