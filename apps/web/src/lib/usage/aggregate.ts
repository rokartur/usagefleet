...
import { modelBreakdown } from './models'
...
export function computeDashboardUsage(
	...
	const sliceFor = (subset: UsageRecord[]): UsageSlice => {
		const session = block ? foldAndSum(filterByWindow(subset, block.start, now)) : { ...EMPTY_TOTALS }
	...
	const modelsFor = (subset: UsageRecord[]): ModelUsage[] => modelBreakdown(filterByWindow(subset, weekStart, now))
