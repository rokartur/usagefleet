 ...
 */
function bucketize(folded: UsageRecord[], start: Date, end: Date, granularity: TimelineGranularity): TimelineBucket[] {
	...
	for (const ev of folded) {
		...
		let cell = acc.get(ck)
		if (!cell) {
			acc.set(ck, (cell = { d: dk, g: gk, m: mk, s: sk, totals: emptyTotals() }))
		}
...
export function buildDailyTimeline(events: UsageRecord[], start: Date, end: Date): TimelineBucket[] {
	return buildTimeline(events, start, end, 'day')
}

/** Hourly series over [start, end] (e.g. the last 24h). */
export function buildHourlyTimeline(events: UsageRecord[], start: Date, end: Date): TimelineBucket[] {
