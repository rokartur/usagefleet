  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-2">
        ...
        <FieldDescription>
          It installs the collector, starts it at login and reads your Claude login on that machine.
...
export function AddDeviceForm({
  ...
  const groupItems = groups.length
    ...
    : [{ value: "", label: "Default (created automatically)" }];
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await toast.promise(createDevice({ data: { name, groupId: groupId || null } }), {
      ...
      setCreated({ token: res.token, name });
      setName("");
        ...
        {created ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                One command left. The token inside it is shown once.
              </DialogDescription>
            </DialogHeader>
