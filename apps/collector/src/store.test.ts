...
describe("store", () => {
  it("folds the three legacy files into one, keeping token and offsets", () => {
    writeFileSync(
      ...
      JSON.stringify({ endpoint: "https://a.test", token: "uf_old", projectsDir: "/p" }),
    );
    writeFileSync(
      join(home, ".usagefleet-state.json"),
      JSON.stringify({ deviceId: "dev-1", files: { "/a.jsonl": { inode: 7, offset: 42 } } }),
    );
    writeFileSync(
    ...
    expect(store.state.files["/a.jsonl"]).toEqual({ inode: 7, offset: 42 });
    expect(store.notify.fiveHour.lastBucket).toBe(80);
    // A window absent from the legacy file still comes back whole.
    expect(store.notify.sevenDay).toEqual({ lastBucket: 0, resetsAt: null });
  });
