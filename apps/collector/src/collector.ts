...
async function sendChunk(
  ...
  if (res.fatal !== "invalid") return res.fatal;
  ...
  if (records.length === 1 && single) {
    if (result.dropped >= dropCeiling) {
      ...
      log(
        `every split of this batch was rejected, so the batch itself is bad, not its records — nothing skipped`,
      );
