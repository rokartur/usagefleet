...
export async function checkForUpdate(
  ...
  if (!res.ok) {
    if (force) log(`update: server has no release info (${res.status}).`);
    return null;
  }
  const latest = (await res.json()) as LatestResponse;
  if (!latest.tag || latest.tag === RELEASE_TAG) {
    if (force) log(`update: already on ${RELEASE_TAG}.`);
    return null;
  }
  ...
  try {
    const dl = await fetch(
      `${cfg.endpoint}/api/v1/collector/download?asset=${encodeURIComponent(asset)}`,
      { headers: { "x-api-key": cfg.token }, signal: AbortSignal.timeout(300_000) },
    );
    if (!dl.ok) {
    ...
    log(`update: ${(err as Error).message}`);
    return null;
  }
