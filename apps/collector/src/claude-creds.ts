...
function fromCredentialsFile(): OAuthBlob | null {
  try {
    return JSON.parse(readFileSync(credentialsFilePath(), "utf8")) as OAuthBlob;
  } catch {
    return null;
  }
}
...
function persist(blob: OAuthBlob, from: "file" | "keychain"): void {
  ...
  if (stored !== json) throw new Error("keychain write did not round-trip");
}
...
async function refreshOauth(
  ...
  try {
    ...
    console.error(
      `could not save the refreshed Claude token (${(err as Error).message}) — ` +
